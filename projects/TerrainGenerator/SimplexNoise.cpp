...
*/
float SimplexNoise::normalDisRandom(float min, float max){
...
float SimplexNoise::simplexPixel(float xin, float yin) {
...
#define G2 0.211324865 // G2 = (3.0-Math.sqrt(3.0))/6.0
	...
	if (t0<0.0f) n0 = 0.0f;
		...
		t0 *= t0;
		n0 = t0 * t0 * SimplexNoise::grad(gi0, x0, y0);  // (x,y) of grad3 used for 2D gradient
	}
	float t1 = 0.5f - x1*x1 - y1*y1;
	if (t1<0.0f) n1 = 0.0f;
	else {
		t1 *= t1;
		n1 = t1 * t1 * SimplexNoise::grad(gi1, x1, y1);
	}
	float t2 = 0.5f - x2*x2 - y2*y2;
	if (t2<0.0f) n2 = 0.0f;
	else {
		t2 *= t2;
		n2 = t2 * t2 * SimplexNoise::grad(gi2, x2, y2);
	}
	// Add contributions from each corner to get the final noise value.
	// The result is scaled to return values in the interval [-1,1].
	float value = 40.0 * (n0 + n1 + n2);
	value = (value + 1.0f) / 2.0f;
	return value;
};

void SimplexNoise::simplex(std::vector<float> &vec, int resolution){

	int c = 0, p = 0;

	for (auto xpos = 0; xpos < resolution ; xpos++){
		for (auto ypos = 0; ypos < resolution ; ypos++){
			float value = 0.0f;
			
			float frequency = 1.0f / (resolution * 0.75f);
			...
			{
				value += (SimplexNoise::simplexPixel(xpos * frequency, ypos * frequency) * amplitude);
				//value += normalDisRandom(-(value /100.0f), value /100.0f);
				frequency *= 2.0f;// lacunarity;
				amplitude *= 0.5f;// gain;
			};
			
			value -= 0.2f;
			//value = SimplexNoise::simplexPixel(xpos, ypos);
			if (value > 1.0f){ value = 1.0f;  c++; }
			else if (value < 0.0f){ value = 0.0f; p++; }
			vec[IDX(xpos, ypos, resolution)] =
				value;
		}
	}

	std::cout << std::endl << c << std::endl << p << std::endl;
	//system("pause");
};
